...
// Public token (meant to be embedded client-side — Mapbox's own model) — kept
// out of source so GitHub's secret scanner doesn't flag it and so it's not
// hardcoded per-environment. Set VITE_MAPBOX_TOKEN in .env.local (dev) and in
// Vercel's project env vars (deploy). Scope it down to this domain in the
// Mapbox dashboard's token settings if this prototype ever gets a fixed URL.
...
/** Real Mapbox GL map (Mapbox's own hosted "light-v11" style — a personal
 * account's free-tier public token, 50k loads/month, no card required) —
 * swaps the old static SVG illustration for an actual live map centered on
 * the user's real city coordinates (see cityCenter/CITY_CENTERS), with
 * numbered pin markers at each result's real (mock-jittered, not geocoded)
 * lat/lng. One live instance per container id; re-mounting destroys the
 * previous one so patchNode-driven re-renders (landmark add/remove) don't
 * leak instances. */
...
// [lat, lng] (mock data's convention) -> Mapbox's [lng, lat].
const toLngLat = ([lat, lng]) => [lng, lat];

// GeoJSON polygon approximating a real-world-meters circle (Mapbox GL's own
